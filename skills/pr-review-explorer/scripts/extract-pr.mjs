...
const meta = JSON.parse(run('gh', 'pr', 'view', pr, '--json', 'number,title,body,url,author,baseRefName,baseRefOid,headRefName,headRefOid,state,isDraft,mergeable,reviewDecision,additions,deletions,changedFiles,comments,createdAt,updatedAt,labels,statusCheckRollup'));
...
const normalizedMeta = { ...meta, comments: Array.isArray(meta.comments) ? meta.comments.length : Number(meta.comments ?? 0) };
writeFileSync(join(destination, 'evidence/pr.json'), `${JSON.stringify({ schemaVersion: 1, repository: repo, pr: normalizedMeta, files }, null, 2)}\n`); chmodSync(join(destination, 'evidence/pr.json'), 0o400);
